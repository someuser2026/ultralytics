...
submit_job() {
  ...
  local classes="$4"
  local imgsz="$5"
  local batch project prefix config task_args optimizer_args varlist job_name

  config="$(config_for "$task" "$classes")"
  [[ -f "$config" ]] || { echo "Missing config: $config"; exit 1; }
  if [[ "$task" == "obb" ]]; then
    batch="$BATCH_OBB"
    project="dino_dataset_imgsz_ablation_obb"
    prefix="o"
    task_args=",ANGLE_MODE=le90"
  else
    batch="$BATCH_SEG"
    project="dino_dataset_imgsz_ablation_segment"
    prefix="s"
