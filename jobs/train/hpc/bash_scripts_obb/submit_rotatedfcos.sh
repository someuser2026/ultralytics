...
usage() {
  cat <<'EOF'
Usage:
  bash jobs/train/hpc/bash_scripts_obb/submit_rotatedfcos.sh [MODEL_OR_CONFIG] [IMGSZ] [BATCH] [MULTISPECTRAL_OPT] [DRY_RUN]

Examples:
  bash jobs/train/hpc/bash_scripts_obb/submit_rotatedfcos.sh
  bash jobs/train/hpc/bash_scripts_obb/submit_rotatedfcos.sh legnet-small-fcos-smallobj 224 16 pn10075s 1
  bash jobs/train/hpc/bash_scripts_obb/submit_rotatedfcos.sh ultralytics/cfg/models/legnet/legnet-small-fcos.yaml 224 16 21 0

Supported aliases:
  legnet-small-fcos
  legnet-small-fcos-smallobj
...
MODEL_OR_CONFIG="${1:-legnet-small-fcos}"
IMGSZ="${2:-224}"
BATCH="${3:-16}"
...
resolve_config() {
  ...
  case "$model_or_config" in
    legnet-small-fcos|legnet-small-fcos.yaml)
      printf 'ultralytics/cfg/models/legnet/legnet-small-fcos.yaml\n'
      return 0
      ;;
    legnet-small-fcos-smallobj|legnet-small-fcos-smallobj.yaml)
      printf 'ultralytics/cfg/models/legnet/legnet-small-fcos-smallobj.yaml\n'
      return 0
      ;;
    legnet-small-fcos-knobtest|legnet-small-fcos-knobtest.yaml)
      echo "legnet-small-fcos-knobtest is test-only and is not supported by this submitter" >&2
      return 1
      ;;
  esac
  ...
  if [[ "$(basename "$resolved")" == "legnet-small-fcos-knobtest.yaml" ]]; then
    echo "legnet-small-fcos-knobtest is test-only and is not supported by this submitter" >&2
    return 1
  fi
...
EXPERIMENT_MODE="${EXPERIMENT_MODE:-${TS}_${SAFE_CONFIG_STEM}_rotatedfcos_${MS_TAG}}"
...
append_var "CLAHE_P" "$CLAHE_P"
append_var "UNSHARP_P" "$UNSHARP_P"
append_var "GAUSSIAN_BLUR_P" "$GAUSSIAN_BLUR_P"
append_var "MOTION_BLUR_P" "$MOTION_BLUR_P"
append_var "MULTI_SPEC_NOISE_P" "$MULTI_SPEC_NOISE_P"
...
echo "============================================================"
echo "Submitting RotatedFCOS job"
echo "Model/config:     ${MODEL_OR_CONFIG}"
echo "Resolved config:  ${CONFIG_YAML}"
echo "Job name:         ${JOB_LABEL}"
echo "Experiment mode:  ${EXPERIMENT_MODE}"
echo "IMGSZ/BATCH:      ${IMGSZ}/${BATCH}"
echo "Multispectral:    ${MULTISPECTRAL_OPT} -> ${MS_CODE} (${MS_DESC})"
echo "Freeze:           ${FREEZE}"
echo "Seed:             ${SEED}"
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[DRY RUN] qsub -V -v \"${VARLIST}\" -N \"${JOB_LABEL}\" \"${PBS_SCRIPT}\""
else
  command -v qsub >/dev/null 2>&1 || { echo "qsub not found on PATH"; exit 1; }
  qsub -V -v "${VARLIST}" -N "${JOB_LABEL}" "${PBS_SCRIPT}"
  sleep 1
