...
: "${RCNN_TASK:?RCNN_TASK must be set by the wrapper}"
: "${DEFAULT_MODEL_ALIAS:?DEFAULT_MODEL_ALIAS must be set by the wrapper}"
: "${DEFAULT_CONFIG_PATH:?DEFAULT_CONFIG_PATH must be set by the wrapper}"
: "${DEFAULT_PROJECT:?DEFAULT_PROJECT must be set by the wrapper}"

SCRIPT_PUBLIC_NAME="${SCRIPT_PUBLIC_NAME:-$(basename "$0")}"

usage_common() {
  cat <<EOF
Usage:
  bash ${SCRIPT_PUBLIC_NAME} [MODEL_OR_CONFIG] [IMGSZ] [BATCH] [MULTISPECTRAL_OPT] [DRY_RUN]
...
resolve_config() {
  ...
  local alias_path=""
  alias_path="$(resolve_config_alias "$model_or_config" || true)"
...
EXPERIMENT_MODE="${EXPERIMENT_MODE:-${TS}_${SAFE_CONFIG_STEM}_${RCNN_TASK}_${MS_TAG}}"

VARS=()
append_var "TASK" "${RCNN_TASK}"
...
echo "============================================================"
echo "Submitting native RCNN ${RCNN_TASK} job"
echo "  Wrapper: ${SCRIPT_PUBLIC_NAME}"
