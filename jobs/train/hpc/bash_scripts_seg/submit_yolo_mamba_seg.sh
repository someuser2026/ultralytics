...
usage() {
  cat <<'EOF'
Usage:
  bash jobs/train/hpc/bash_scripts_seg/submit_yolo_mamba_seg.sh [MODEL_OR_CONFIG] [IMGSZ] [BATCH] [MULTISPECTRAL_OPT] [DRY_RUN]
...
MODEL_OR_CONFIG="${1:-yolo-mamba-seg}"
IMGSZ="${2:-224}"
BATCH="${3:-8}"
...
PROJECT="${PROJECT:-mamba_yolo_segment}"
WANDB="${WANDB:-true}"
...
EXPERIMENT_MODE="${EXPERIMENT_MODE:-${TS}_${SAFE_CONFIG_STEM}_segment_${MS_TAG}}"
...
echo "============================================================"
echo "Submitting YOLO-Mamba segment job"
