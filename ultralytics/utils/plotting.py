...
def plot_results(file: str = "path/to/results.csv", dir: str = "", on_plot: Callable | None = None):
    """
    Plot training results from one or more 'results*.csv' files in the same directory.
    Saves 'results.png' next to the CSV(s).
    """
    import math
    from pathlib import Path
    import numpy as np
    ...
    files = sorted(save_dir.glob("results*.csv"))
    ...
    for fi, f in enumerate(files):
        try:
            ...
            # Build plotting structure once from first file
            if fi == 0:
                cols = data.columns
                loss_keys = [c for c in cols if "loss" in c.lower()]
                metric_keys = [c for c in cols if "metric" in c.lower()]
                
                # Interleave loss/metric halves
                if loss_keys or metric_keys:
                    loss_mid = len(loss_keys) // 2
                    metric_mid = len(metric_keys) // 2
                    columns = (
                        loss_keys[:loss_mid]
                    ...
                    # Fallback: all columns except first
                    columns = cols[1:]
                
                # Create subplot grid
                n = len(columns)
                ncols = math.ceil(n / 2)
                fig, ax = plt.subplots(2, ncols, figsize=(max(6, ncols * 3), 6), tight_layout=True)
            ...
            for i, col in enumerate(columns):
                ...
                # Plot actual values
                a.plot(x, y, marker=".", label=f.stem, linewidth=2, markersize=8)
                
                # Plot smoothed line if enough points
                if len(y) >= 5:
                    a.plot(x, gaussian_filter1d(y, sigma=3), ":", label="smooth", linewidth=2)
                
                # Set title only once (on first file)
                if fi == 0:
    ...
    if fig is not None:
        ...
        legend_idx = min(1, len(columns) - 1) if len(columns) > 1 else 0
...
def feature_visualization(x, module_type: str, stage: int, n: int = 32, save_dir: Path = Path("runs/detect/exp")):
    ...
    for m in {"Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "CascadeRCNNHead", "CascadeMaskRCNNHead"}:  # all model heads
        if m in module_type:
            return
